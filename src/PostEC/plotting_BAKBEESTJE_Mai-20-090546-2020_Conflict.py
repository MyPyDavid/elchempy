...
class eisplot:
    ...
    def PlotParsEIS(AllData_E_file, EISovv, SampleCode, DestFile, yEIS="Rct"):
    ...
    def PlotCombinedEIS(
        ...
        #    maxLim = (AllData_E_file[['DATA_Yim','DATA_Yre']].max()).max()
        #    AllData_E_file_spectras,EISovv,SampleCode,DestFile = spectras_comb, bgrp,SampleCodes, DestFile
        #%%
        EvRHE = "E_AppV_RHE"
        AllData_E_file = AllData_E_file_spectras.loc[
            AllData_E_file_spectras.Model_EEC.isin(EISovv.Model_EEC.unique())
        ]
        if xEIS in ["Y", "Z"]:
        ...
        fig.suptitle(
            ...
            % (
                ...
                Path(EISovv["PAR_file"].unique()[0]).stem,
            )
        )
        dataC, fitC, extraC, initC = "tab:blue", "tab:red", "gold", "gray"
        #    ax.set_xlim(0,maxYre)
    ...
    def plot_par_with_Ev(mgrp, SampleCodes, OriginColor, DestFilePars):
        #        OriginColors = Characterization_TypeSetting.OriginColorList()
        EvRHE = "E_AppV_RHE"
        plst = eisplot.parlst
        plst += [i for i in eisplot.extrapars if i in mgrp.columns]
        #

        for par in plst:
            ht, wd = 10, 10
            ylim, logy, logyscale = eisplot(par).pars_plot_lims()
            fig, ax = plt.subplots(figsize=(ht, wd))
            ...
            plt.savefig(Path(f"{DestFilePars}_{par}.png"), dpi=200, bbox_inches="tight")
            plt.close(fig)
