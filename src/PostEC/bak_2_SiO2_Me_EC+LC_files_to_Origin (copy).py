...
class prepare_EC_merged:
    ...
    ORR_pars_all = Load_from_Indexes.ORR_pars_OVV(**ORR_no_reload)
    mcols = [i for i in Load_from_Indexes.EC_label_cols if i not in ["PAR_file"]] + [
        "Sweep_Type"
    ]

    def __init__(self, reloadset=True):
        self._reloadset = reloadset

        self.load_EC_PorphSiO2()

    def load_EC_PorphSiO2():
                ...
                for match in union:
                    ...
                    for (_prePF, npr, _preAST), prgrp in _mpre:
                        ...
                        for (_poPF, npo, _postAST), pogrp in _mpost:
                            ...
                            if _postAST in "postAST_sHA|postAST_LC":
                                print(_res)
    ...
    def mergedEC(_reloadset=False):
        ...
        if _reloadset == True:
            ...
            ORR_AST_diff = EC_PorphSiO2.compare_AST_pars(
                ORR, ORR_AST, reload=_reloadset
            )
            ...
            tt_diff = EC_PorphSiO2.compare_AST_pars(
                ORR, tt, reload=_reloadset, save_pkl=False
            )
            ...
            EIS_AST_diff = EC_PorphSiO2.compare_AST_pars(
                EIS, EIS_AST, reload=_reloadset
            )
            ...
            for Htype, Hgrp in HER_type_grp:
                ...
                HER_AST_diff = EC_PorphSiO2.compare_AST_pars(
                    Hgrp, HER_AST, reload=_reloadset, extra=Htype
                )
                try:
    ...
    def get_ORR_experiments(EC_merged_dict):
        ORR_AST = EC_merged_dict["ORR"]["AST_matches"]
    ...
    def reproducibility_check_samples(_DF, ECname):
        ...
        if ECname == "N2":
            ...
            _grpcols = ["pH", "Loading_cm2", "SampleID", "Sweep_Type_N2"]
            _sIDgrps = _DF.loc[
            ...
            def select_sID_N2():
                ...
                _raw_data_folder = mkfolder(
                    EC_folder.joinpath(
                        f"{ECname}_reproducibility",
                        "_".join([str(i) for i in _grp_select]),
                    )
                )
                ...
                j4post = pd.concat(
                    [
                        pd.read_excel(
                            _j4lc.SourceFilename.unique()[0].parent.joinpath(i)
                        )
                ...
                j4no = pd.concat(
                    [
                        pd.read_excel(
                            _j4no.SourceFilename.unique()[0].parent.joinpath(i)
                        )
                ...
                for pf, pgr in _j4no_pfgrps:
                    j4no_grps = pd.concat(
                        [
                            pd.read_excel(
                                pgr.SourceFilename.unique()[0].parent.joinpath(i)
                            )
    ...
    def add_filter_selection_of_EC_merged(EC_merged_dict):
        ...
        for _EC, _DF in EC_merged_dict.items():
            ...
            EC_merged_dict.update({_EC: {**_DF, **{"AST_diff_filter": _filtered}}})
        print(
    ...
    def compare_AST_pars(_DF, _AST_in, reload=False, extra="", save_pkl=True):
    ...
    def sense_DF_type(_DF):
        ...
        _excl = set(EC_PorphSiO2.EC_idx_PorphSiO2.columns).union(SampleCodes.columns)
        _res = [
            i
            for i in Counter(
    ...
    def HER_pars(
        ...
        if not Pfolder.joinpath("HER_orig_data.pkl").exists() or reload == True:
            HER_pars_all = Load_from_Indexes.HER_pars_OVV(
                reload=reload, use_daily=use_daily, extra_plotting=extra_plotting
            )
            HER_pars = HER_pars_all.loc[
                (
                    (HER_pars_all._type == "HER_pars")
                    & (HER_pars_all.PAR_file.isin(self.index.PAR_file.to_numpy()))
                )
            ]
    ...
    def ORR_pars(self):
        ...
        ORR_pars = EC_PorphSiO2.ORR_pars_all.loc[
            (
                (EC_PorphSiO2.ORR_pars_all.source_type == "ORR_pars")
                & (
    ...
    def KL_pars(self):
        ...
        KL_pars = EC_PorphSiO2.ORR_pars_all.loc[
            (
                (EC_PorphSiO2.ORR_pars_all.source_type == "KL_pars")
                & (
                    EC_PorphSiO2.ORR_pars_all.PAR_file.isin(
                        self.index.PAR_file.to_numpy()
                    )
    ...
    @edit_columns
    def EIS_pars(_source="Load pars"):
        ...
        if "files" in _source:
            ...
            EIS_pars_mod = EIS_pars.loc[
                EIS_pars.Model_EEC.isin(EC_PorphSiO2.EIS_models.values())
            ]
        ...
        N2grp = ("N2", EC_PorphSiO2.EIS_models.get("N2"))
        O2grp = ("O2", EC_PorphSiO2.EIS_models.get("O2"))
        EP_N2, EP_O2 = EPgrp.get_group(N2grp).drop(columns="Gas"), EPgrp.get_group(
            O2grp
        ).drop(columns="Gas")
        ...
        for gas in ["O2", "N2"]:
            gasn_grp = (gas, EC_PorphSiO2.EIS_models.get(gas))
            grp = EPgrp.get_group(gasn_grp)
