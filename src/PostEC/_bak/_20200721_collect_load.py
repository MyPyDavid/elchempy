...
class Load_from_Indexes:
    ...
    def ORR_pars_OVV(reload=False, extra_plotting=False, xls_out=False):
        ...
        if IndexOVV_ORRpars_fn.exists() and reload is not True:
            ...
            if orr_daily.get("_raw_exists", False):
            ...
            elif orr_daily.get("daily_options", False):
                ...
                ORR_pars_recent = ORR_pars_all.loc[
                    ORR_pars_all.Analysis_date > dt.datetime.fromisoformat("2020-07-15")
                ]
                for n, gr in ORR_pars_recent.groupby("PAR_file"):
                    print(
                        n,
                        f'\nSamples: {", ".join([str(i) for i in gr.SampleID.unique()])}',
                        ",".join(gr._type.unique()),
                    )
                ORR_pars_recent.to_pickle(orr_daily["daily_path_RAW"])
            ...
            ORR_pars = pd.merge(ORR_pars_recent, SampleCodes, on="SampleID", how="left")
