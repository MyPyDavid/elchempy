...
def integrand_L2_im(x, xi, xi_prime, sigma_f, ell):
    f = np.exp(xi)
    f_prime = np.exp(xi_prime)
    # ell, x, xi, xi_prime = (-108.72805905025372, 935.2606747597932, -0.6931471805599453, -0.6931471805599453)
    try:
        numerator = np.exp(x - 0.5 / (ell ** 2) * (x ** 2)) * (x + xi_prime - xi)
        if x < 0:
            numerator = np.exp(x - 0.5 / (ell ** 2) * (x ** 2)) * (x + xi_prime - xi)
            denominator = -1 + ((f_prime / f) ** 2) * exp(2 * x)
        else:
            numerator = np.exp(-x - 0.5 / (ell ** 2) * (x ** 2)) * (x + xi_prime - xi)
            denominator = -np.exp(-2 * x) + ((f_prime / f) ** 2)
    except Exception as e:
        print(f"ell, x, xi, xi_prime = ({ell}, {x}, {xi}, {xi_prime}),\nerror:{e}")
...
def integrand_der_ell_L2_im(x, xi, xi_prime, sigma_f, ell):
    ...
    try:
        if x < 0:
            numerator = (
                (x ** 2) * exp(x - 0.5 / (ell ** 2) * (x ** 2)) * (x + xi_prime - xi)
            )
            denominator = -1 + ((f_prime / f) ** 2) * exp(2 * x)
        else:
            numerator = (
    ...
    if np.array_equal(xi_n_vec, xi_m_vec):
        ...
        for n in range(0, N_freqs):
            ...
            integral, tol = integrate.quad(
                ...
                epsabs=1e-12,
                epsrel=1e-12,
                args=(xi, xi_prime, sigma_f, ell),
            )

            np.fill_diagonal(
                L2_im_K[n:, :], exp(xi_prime - xi) * (sigma_f ** 2) * integral
            )
            np.fill_diagonal(
                L2_im_K[:, n:], exp(xi_prime - xi) * (sigma_f ** 2) * integral
            )
...
def der_ell_matrix_L2_im_K(xi_vec, sigma_f, ell):
    ...
    for n in range(0, N_freqs):
        ...
        np.fill_diagonal(
            der_ell_L2_im_K[n:, :],
            exp(xi_prime - xi) * (sigma_f ** 2) / (ell ** 3) * integral,
        )
        np.fill_diagonal(
            der_ell_L2_im_K[:, n:],
            exp(xi_prime - xi) * (sigma_f ** 2) / (ell ** 3) * integral,
...
def NMLL_fct(theta, Z_exp, xi_vec):
    ...
    L2_im_K = matrix_L2_im_K(
        xi_vec, xi_vec, sigma_f, ell
    )  # $\mathcal L^2_{\rm im} \mathbf K$
    K_im_full = (
        L2_im_K + Sigma
    )  # $\mathbf K_{\rm im}^{\rm full} = \mathcal L^2_{\rm im} \mathbf K + \sigma_n^2 \mathbf I$
    L = np.linalg.cholesky(
        K_im_full
    )  # Cholesky decomposition to get the inverse of $\mathbf K_{\rm im}^{\rm full}$

    # solve for alpha
    alpha = np.linalg.solve(L, Z_exp.imag)
    alpha = np.linalg.solve(
        L.T, alpha
    )  # $(\mathcal L^2_{\rm im} \mathbf K + \sigma_n^2 \mathbf I)^{-1} \mathbf Z^{\rm exp}_{\rm im}$
...
def grad_NMLL_fct(theta, Z_exp, xi_vec):
    ...
    der_mat_sigma_n = (2.0 * sigma_n) * np.eye(
        N_freqs
    )  # derivative of $\sigma_n^2 \mathbf I$ to $sigma_n$
    der_mat_sigma_f = (
        2.0 / sigma_f
    ) * L2_im_K  # note that the derivative is 2*sigma_f*L2_im_K/(sigma_f**2)
    der_mat_ell = der_ell_matrix_L2_im_K(
        xi_vec, sigma_f, ell
