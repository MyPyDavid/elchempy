...
def CDL(N2_CVs, scanrates):
    ...
    if False:
        for ycol in ['lin_slope']:
            Cdl_data.query('lin_zscore_y < 3').groupby("SweepType").plot(x=EvRHE,y='lin_intercept')

    Cdl_data.query('lin_zscore_y < 3')
    if False:
        Resmean = Cdl_data.groupby([EvRHE,'SweepType',"scanrate"]).mean()
        Resmean = Resmean.query(
...
def check_for_linear_baseline_correction_of_Cdl_values(Cdl_data):
    ...
    for swpnm, swpgrp in Cdl_data.groupby("SweepType"):
            ...
            lin_slope_baseline_corr = swpgrp["lin_slope"] - lin_baseline_model + swpgrp["lin_slope"].mean()
...
def prepare_Cdl_frame(N2_scans,
                      current_density_key = "j A/cm2",
                      potential_key = EvRHE):
# Loop over Sweep Types
    _results = []
    for (sr, swpname),swpgrp in N2_scans.query('scanrate > 0').groupby(['scanrate','SweepType']):
    ...
    # Loop over potentials E This mean for example for E in [0.1, 0.3, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7,0.725,0.75,0.775,0.8,0.825,0.85,0.9]:
        for E in np.linspace(0.1, 1, 50):
            ...
            j_Cdl = np.abs(
            swpgrp.loc[
            (np.isclose(swpgrp[EvRHE], E, atol=0.010))
            ][current_density_key]
            ).mean()
