...
class RDE_Plotter:
    ...
    def plot_all_scans_scanrate(
        ...
        for sr, sgr in plot_data.groupby("Segment #"):
    ...
    def plot_ring_disk(
        ...
        Iring_Chrono=pd.DataFrame(),
        _ORR_RRDE_pars=pd.DataFrame(),
        ORR_dest_dir_file=None,
    ):
        ...
        O2_join_swgrp = O2_join.groupby("SweepType")
        ...
        if not _ORR_RRDE_pars.empty:
            ORR_ring_file_name = _ORR_RRDE_pars.Ring_PAR_file.unique()[0].name
            _OCP_ring = _ORR_RRDE_pars.Ring_Measured_OCP.unique().mean()
        else:
            ORR_ring_file_name = "no_ring_file_given"
            _OCP_ring = 0

        if not Iring_Chrono.empty:
        ...
        if not ORR_disk_pars.empty:
            ...
            cath_pars = (
                _cath_ORR_pars[
                    [
                        i
                        for i in _cath_ORR_pars.columns
                        if _cath_ORR_pars[i].nunique() == 1
                    ]
                ]
                .iloc[0]
                .to_dict()
...
#     self.pars.groupby("SweepType").plot(
#         x=EvRHE, y="lin_slope", kind="scatter"
#     )
