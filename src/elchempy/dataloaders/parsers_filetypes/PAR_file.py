...
class VersaStudioParser(HTMLParser):
    ...
    def parse_text(self, text, current_tag):
        ...
        current_tag_name = current_tag
        # if 'instrument' in current_tag:
        #     breakpoint()

        if any(i in current_tag for i in ["mode:floating", "filter:normal"]):
            # current_tag_bak = current_tag
            if "filter:normal" in current_tag:
                # breakpoint()
                raw_data_modefloat = self._all_raw_data.get(
                    "mode:floating,", "None"
                ).strip()
                tagname = "".join([i for i in raw_data_modefloat if i.isalpha()])
            else:
                tagname = "mode"
            current_tag_name = "instrument_" + tagname
            ...
            if starttag_text:
                strt_text = (
                    "\n"
                    + starttag_text[1:-1].replace(":", "=").replace(",", "\n")
                    + "\n"
                )
                textstrp = textstrp + strt_text

        textdict = {current_tag_name: None}

        try:
            # splt = [i.split("=") for i in text[1:-1].split("\n")]
            splitted_lines = [
                line.split(sep="=")
                for line in textstrp.splitlines()
                if (line != "" and "=" in line)
            ]
