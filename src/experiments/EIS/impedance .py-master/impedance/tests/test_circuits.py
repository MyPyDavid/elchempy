...
def test_Randles():
    ...
    assert np.isclose(
        randles.predict(np.array([10.0])), np.complex(0.02495749, -0.00614842)
    )
    ...
    randles1 = Randles(initial_guess=[0.01, 0.005, 0.1, 0.0001, 200])
    randles2 = Randles(initial_guess=[0.01, 0.005, 0.1, 0.0001, 200])
...
def test_CustomCircuit():
    ...
    custom_circuit = CustomCircuit(initial_guess=initial_guess, circuit=custom_string)
    ...
    high_f = np.array([1e9])
    assert np.isclose(np.real(custom_circuit.predict(high_f)[0]), initial_guess[0])

    # __str()__
    initial_guess = [0.01, 0.005, 0.1]
    ...
    initial_guess = [0.005, 0.1, 0.001]
    custom_circuit = CustomCircuit(
        initial_guess=initial_guess, constants=constants, circuit=circuit, name="Test"
    )
    ...
    with pytest.raises(ValueError):
        ...
        custom_circuit = CustomCircuit(
            initial_guess=initial_guess, circuit=custom_string
        )
