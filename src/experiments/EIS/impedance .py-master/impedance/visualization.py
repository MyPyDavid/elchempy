...
def plot_nyquist(ax, Z, scale=1, units="Ohms", fmt=".-", **kwargs):
    ...
    **kwargs : `matplotlib.pyplot.Line2D` properties, optional
    ...
    ax.set_xlabel(r"$Z^{\prime}(\omega)$ " + "$[{}]$".format(units), fontsize=20)
    ax.set_ylabel(r"$-Z^{\prime\prime}(\omega)$ " + "$[{}]$".format(units), fontsize=20)
...
def plot_bode(axes, f, Z, scale=1, units="Ohms", fmt=".-", mag_log=False, **kwargs):
    ...
    **kwargs : `matplotlib.pyplot.Line2D` properties, optional
    ...
    if mag_log:
        ax_mag.plot(f, np.log10(np.abs(Z)), fmt, **kwargs)
        ax_mag.set_ylabel(r"$log(|Z(\omega)|)$ " + "$[{}]$".format(units), fontsize=20)
    else:
        ax_mag.plot(f, np.abs(Z), fmt, **kwargs)
        ax_mag.set_ylabel(r"$|Z(\omega)|$ " + "$[{}]$".format(units), fontsize=20)

    _Zangle = -np.angle(Z, deg=True)
    # _Zangle =  np.tan(np.abs(Z.real)/np.abs(Z.imag))
    ax_phs.plot(f, _Zangle, fmt, **kwargs)
    ...
    limits = -np.log10(scale)
    if limits != 0:
        ax_mag.ticklabel_format(style="sci", axis="y", scilimits=(limits, limits))
    y_offset = ax_mag.yaxis.get_offset_text()
    y_offset.set_size(18)
    ax_phs.set_ylim(-25, 65)
...
def plot_altair(data_dict, size=400, background="#FFFFFF"):
    ...
    for kind in data_dict.keys():
        ...
        df = pd.DataFrame(
            {"f": f, "z_real": Z.real, "z_imag": Z.imag, "kind": kind, "fmt": fmt}
        )
    ...
    nearest = alt.selection_single(
        on="mouseover", nearest=True, empty="none", fields=["f"]
    )
    ...
    if "-" in fmts:
        ...
        nyquist = (
            ...
            .encode(
                x=alt.X(
                    ...
                    scale=alt.Scale(domain=[min_x, max_x], nice=False, padding=5),
                ),
                y=alt.Y(
                    "neg_z_imag:Q",
                    axis=alt.Axis(title="-Z'' [Ω]"),
                    scale=alt.Scale(domain=[min_y, max_y], nice=False, padding=5),
                ),
                color="kind:N",
            )
            .properties(height=size, width=size)
            .transform_calculate(neg_z_imag="-datum.z_imag")
        )
        ...
        bode = (
            ...
            .encode(
                alt.X(
                    "f:Q",
                    axis=alt.Axis(title="f [Hz]"),
                    scale=alt.Scale(type="log", nice=False),
                ),
                color="kind:N",
            )
            .properties(width=size, height=size / 2 - 25)
            .transform_calculate(
        ...
        bode_mag = bode.encode(y=alt.Y("mag:Q", axis=alt.Axis(title="|Z| [Ω]")))
        bode_phs = bode.encode(y=alt.Y("neg_phase:Q", axis=alt.Axis(title="-ϕ [°]")))
...
def plot_residuals(
    ax, f, res_real, res_imag, fmt=".-", y_limits=(-5, 5), extra_label="", **kwargs
):
    ...
    **kwargs : `matplotlib.pyplot.Line2D` properties, optional
    ...
    ax: matplotlib.axes.Axes
    """
    re_lbl = r"$\Delta_{\,\mathrm{Re}}$"
    im_lbl = r"$\Delta_{\,\mathrm{Im}}$"
    if extra_label:
        re_lbl = f"{extra_label}" + r"$\Delta_{\,\mathrm{Re}}$"
        im_lbl = f"{extra_label}" + r"$\Delta_{\,\mathrm{Im}}$"
    #        r'\{0\}$\Delta_{\,\mathrm{Re}}$'.format(extra_label)
    ax.plot(f, res_real * 100, fmt, label=re_lbl, **kwargs)
    ax.plot(f, res_imag * 100, fmt, label=im_lbl, **kwargs)
