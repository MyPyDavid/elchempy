...
def test_eval_linKK():
    ...
    assert np.isclose(
        residuals_linKK(Rs, ts, Z_data, f), (Z_data - Z).real / np.abs(Z_data)
    ).all()

    assert np.isclose(
        residuals_linKK(Rs, ts, Z_data, f, residuals="imag"),
        (Z_data - Z).imag / np.abs(Z_data),
    ).all()

    diff_real = (Z_data - Z).real / np.abs(Z_data)
    diff_imag = (Z_data - Z).imag / np.abs(Z_data)
    assert np.isclose(
        residuals_linKK(Rs, ts, Z_data, f, residuals="both"),
        [diff_real[0], diff_imag[0], diff_real[1], diff_imag[1]],
    ).all()
...
def test_get_tc_distribution():
    M = 3
    f = np.array([1000, 100, 10, 1, 0.1]) / (2 * np.pi)
...
def test_linKK():
    ...
    M, mu, Z_fit, resids_re, resids_im = linKK(f, Z, c=0.69)
    ...
    M, mu, Z_fit, resids_re, resids_im = linKK(f, Z, c=0.69, add_cap=True)
    ...
    M, mu, Z_fit, resids_re, resids_im = linKK(f, Z, c=None, max_M=10, add_cap=True)
