...
from .N2.analyze_scans import N2_analyze_scan
...
def add_missing_create_CVs(N2act_ovv,index_fls = {},filter_out = ['_1cls_', '_AST_20.000'] ):

    _df = index_fls['N2_CVs']['data']
    set_pf_ovv, set_df = set([Path(i) for i in N2act_ovv.PAR_file.unique()]), set(_df.PAR_file.unique())
    if set_pf_ovv <= set_df:
        return index_fls
    else:
        _leftover = set_pf_ovv.difference(set_df)
        _leftfltrd = [i for i in _leftover if not re.search('|'.join(filter_out),i.name)]
        if _leftfltrd:
            _N2left = N2act_ovv.loc[N2act_ovv.PAR_file.isin([str(i) for i in _leftfltrd])]
            N2_CVs_all, N2_actions_all = create_CVs(_N2left)
            _CVs_add = pd.concat([_df,N2_CVs_all], sort=False,ignore_index=True)
            _actions_add = pd.concat([index_fls['N2_actions']['data'],N2_actions_all], sort=False,ignore_index=True)

            for k,val in index_fls.items():
#                _df_add = pd.concat([_df,N2_CVs_all], sort=False,ignore_index=True)
                if 'N2_actions' == k:
                    index_fls[k].update({'data' : _actions_add,'PAR_files' : set(_actions_add.PAR_file.unique())})
                    _action_add.to_pickle(val['path'])
                elif 'N2_Cvs' == k:
                    index_fls[k].update({'data' : _CVs_add,'PAR_files' : set(_CVs_add.PAR_file.unique())})
...
#            EIS_ovv['PAR_file'] = [Path(i) for i in EIS_ovv['PAR_file'].to_numpy()]
            index_fls[k].update({'data' : _df,'PAR_files' : set(_df.PAR_file.unique())})
    return index_fls
...
def load_all_N2_CVs(N2act_ovv):
    ...
    CV_data_local = add_missing_create_CVs(N2act_ovv,index_fls = CV_data_local,filter_out = ['_1cls_', '_AST_20.000'] )
...
#                    Cdl_fit_pars, Cdl_fit_data = pd.DataFrame(), pd.DataFrame()
    N2act_ovv = ovv_exp_grp.get_group('N2_act').drop_duplicates(subset=['PAR_file','PAR_hash'],keep='first')
...
#            EISgrEvRHE_data_grp = EISgrEvRHE_data_raw.groupby(['PAR_file',EvRHE, 'RPM_DAC'])
    ...
    #            EIS_run_grps = [(nm,gr,EIS_kwargs) for nm,gr in EIS_ovv.groupby(by='PAR_file')]
            logger.info(f'N2 run_group_ovv START multiprocessing {multi_par_fit} for len{len(fit_run_args)}')
...
#            for chunk in fit_run_args[:pool_size if pool_size > 2 else 1]:
            with multiprocessing.Pool(pool_size) as pool:
                fit_export_EV_all_chunck = PF_fit_starmap_with_kwargs(pool, N2_scans, fit_run_args, fit_kwargs_iter)
...
#                        N2_bgs_lst.append(N2_background)
                    logger.info(f'N2 fit_run_arg background succes: {fit_run_arg.PAR_file}')
                else:
                    logger.warning(f'N2 fit_run_arg background empty for: {fit_run_arg.PAR_file}')
#                        Cdl_fit_pars = pd.concat([Cdl_fit_pars,Cdl_PARS])
#                        Cdl_fit_data = pd.concat([Cdl_fit_data,Cdl_dt])
#                    print('N2 background succes!')
            except Exception as e:
#                print('N2 FAIL',e)
                logger.error(f'ERROR N2 run_fit_arg FAIL: {fit_run_arg.PAR_file}.\nbecause  {e}')
                N2_background,Cdl_dt,Cdl_PARS = pd.DataFrame([]),pd.DataFrame([]),pd.DataFrame([])
                logger.warning('N2_act fail: {0}\n because {1}'.format(N2_file,e))
